...
\node[consistency execution, above left=0.3*\distance and 2.5*\distance of input_m2.north, anchor=north, align=center] {Consistency\\ Relations};
...
\draw[consistency execution, double distance=0.15em, -latex] ([xshift=1em]changes.east) -- node[below] {Application Function} ([xshift=-1em]output_m1.west);

% ORCHESTRATION FUNCTION
\draw[consistency execution, decorate,decoration={brace,amplitude=10pt},xshift=0pt,yshift=-4pt] 
...
\draw[consistency execution, double distance=0.15em, -latex] 
    ([yshift=-10pt]brace_midpoint) 
    |- 
    node[pos=0.75,below] (orchestration_function) {Orchestration Function} 
    ++(2.5*\distance,-0.8*\distance);

% CONSISTENCY PRESERVATION
\network[below left=5.7*\distance and 1.5*\distance of input_m1.center, anchor=center]{cprs}{transformation}{$\consistencypreservationrule{1}$}{$\consistencypreservationrule{2}$}{$\consistencypreservationrule{3}$}{$\consistencypreservationrule{4}$}
\node[consistency execution, above=0.3em of cprs_m2.north, anchor=south, align=center] {Consistency\\ Preservation Rules};
...
\coordinate (orchestration_start) at ([xshift=3*\distance, yshift=-2.5\distance]brace_midpoint);
\draw[transformation] 
...
% LABELS
\node[above right=0.5em and 1.5*\distance of input_m2.north, anchor=south, font=\bfseries\small] {Process Inputs};
