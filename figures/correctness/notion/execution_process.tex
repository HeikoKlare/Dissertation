...
\newcommand{\hdistancebase}{2em}
\newcommand{\hdistancecorrection}{0.08*\difftoafiveimage}
...
\draw[consistency execution, double distance=0.15em] 
    ...
    ++(3*\hdistancebase+3*\hdistancecorrection,-1*\vdistance);

% CONSISTENCY PRESERVATION
\network[below left=5.7*\vdistance and 1.7*\hdistance of input_m1.center, anchor=center]{cprs}{transformation}{$\consistencypreservationrule{1}$}{$\consistencypreservationrule{2}$}{$\consistencypreservationrule{3}$}{$\consistencypreservationrule{4}$}
...
\coordinate (orchestration_start) at ([xshift=3*\hdistance, yshift=-2.7\vdistance]brace_midpoint);
\draw[transformation] 
    ...
    ++(0.6*\hdistancebase+0.6*\hdistancecorrection, 0.6*\vdistance);
\draw[transformation] 
