...
\begin{lstlisting}[language=framedQVT,linewidth=5.5cm, mathescape=true]
Person::firstname = fstn
$\land$ Person::lastname = lstn
$\land$ Employee::name = fstn + ' ' + lstn
\end{lstlisting}
        };
...
\begin{lstlisting}[language=framedQVT,linewidth=4cm]
(not
  (and
    (= firstname fstn)
    (= lastname lstn)
...
\end{lstlisting}
    ...
    \node[above=-0.3cm of smtEmpRes] (wedge1) {$\land$};
    \node[above=-0.3cm of smtPersRes] (wedge2) {$\land$};
