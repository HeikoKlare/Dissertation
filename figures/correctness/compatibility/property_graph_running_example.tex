\begin{tikzpicture}[
    ...
    \node[above = 0.3em of circleResN] {$\mathvariable{Resident.name}$};

    \node[metagraph node, name=circleEmpN] at ($(18em, -2em)$) {};
    \node[above = 0.3em of circleEmpN] {$\mathvariable{Employee.name}$};

    \node[metagraph node, name=circlePersF] at ($(14em, 8em)$) {};
    \node[below = 0.3em of circlePersF] {$\mathvariable{Person.firstname}$};

    \node[metagraph node, name=circlePersN] at ($(12em, 10em)$) {};
    \node[above = 0.3em of circlePersN] {$\mathvariable{Person.lastname}$};
    
    %% METAGRAPH NODES (part 2)
    \node[metagraph node, name=circlePersA] at ($(8em, 12em)$) {};
    \node[above = 0.5em of circlePersA] {$\mathvariable{Person.address}$};

    \node[metagraph node, name=circlePersI] at ($(21em, 8em)$) {};
    \node[above = 0.5em of circlePersI] {$\mathvariable{Person.income}$};
    
    \node[metagraph node, name=circleEmpSo] at ($(16em, -6em)$) {};
    \node[above = 0.2em of circleEmpSo] {$\mathvariable{Employee.socsecnumber}$};
    
    \node[metagraph node, name=circleEmpSa] at ($(22em, 0)$) {};
    \node[above left = 0.3em and -3.7em of circleEmpSa] {$\mathvariable{Employee.salary}$};
    
    \node[metagraph node, name=circleResA] at ($(-2em, 4em)$) {};
    \node[above right = 0.3em and -2.5em of circleResA] {$\mathvariable{Resident.address}$};
    
    \node[metagraph node, name=circleResS] at ($(2em, -4em)$) {};
    \node[above = 0.3em of circleResS] {$\mathvariable{Resident.socsecnumber}$};
