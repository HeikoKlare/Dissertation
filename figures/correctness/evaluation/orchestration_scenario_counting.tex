...
\umlclassvarwidth[, above=0.55*\distance of B.north, anchor=north]{D}{}{D}{
n
}{\classwidth}
...
\draw[consistency relation]
    ...
    node[above, align=center] {$\consistencyrelation{CR}{AB} = $\\ %}
    %node[below, align=center] {
        $\{\tupled{a,b} \mid a.n, b.n \geq 0$ \\ $\land \; b.n = a.n + 1 \land b.n \neq x\}$}
    ([yshift=0.7em]B.west);
\draw[consistency relation]
...
\draw[consistency relation]
    ...
    node[above, align=center] {$\consistencyrelation{CR}{BC} = $ %}
    %node[below] {
        $\setted{\tupled{b,c} \mid b.n = c.n}$}
    ([yshift=0.7em]C.west);
\draw[consistency relation]
