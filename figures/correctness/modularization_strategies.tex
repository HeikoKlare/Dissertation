...
\begin{tikzpicture}[
    ...
    consistencyrel/.style={latex-latex,consistency related element},
...
\draw[consistencyrel] (full_left) -- (full_top);
\draw[consistencyrel] (full_left) -- (full_right);
\draw[consistencyrel] (full_left) -- (full_bottom);
\draw[consistencyrel] (full_top) -- (full_right);
\draw[consistencyrel] (full_top) -- (full_bottom);
%\draw[consistencyrel] (full_top) to[bend left=50] ([xshift=1em]full_right) to [bend left=50] (full_bottom);
\draw[consistencyrel] (full_right) -- (full_bottom);
\draw[consistencyrel] (full_left) -- (full_bottomleft);
\draw[consistencyrel] (full_bottom) -- (full_bottomleft);
\draw[consistencyrel] (full_top) to[bend right=30] (full_bottomleft);
\draw[consistencyrel] (full_bottomleft) .. controls ++(7em, -1em) and ([yshift=-3em, xshift=-0.2em]full_right.south) .. (full_right);
%\draw[consistencyrel] (full_bottomleft) to[bend right=35] ([xshift=0.6*\mmdistance]full_bottom) to[bend right=20] (full_right);
...
\node[draw=darkgray, matrix, legend, nodes=mininode, below right=0em and 2*\hmmdistance of tree_top, anchor=north west, outer sep=0, inner sep=0.4em, column sep=0.4em, row sep=0.2em] (legend) {
    ...
    %
    \draw[consistencyrel] (-0.7em,0) -- (0.7em,0); &
...
% \draw[consistencyrel] ([yshift=-2em, xshift=-0.7em]legend_mm.center) -- ([yshift=-2em, xshift=0.7em]legend_mm.center);
