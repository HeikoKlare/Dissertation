...
\node[mmlabel, anchor=center] (uml_label) at (uml_label_coordinate) {UML};
...
\draw[consistency relation] ([xshift=1.5em]java_class.south) 
    -- 
    %node[pos=0, below right] {$jclass$} 
    ++(0,-2em) 
    -| 
    node[pos=0.25, below, align=center] {$\mathvariable{jclass.packageName} = \mathvariable{ucomp.name}$ \\ $\mathvariable{jclass.name} = \mathvariable{ucomp.name} + \text{\enquote{Impl}}$} 
    %node[pos=1, below left] {$ucomp$} 
([xshift=-1.5em]uml_component.south);
\draw[consistency relation] ([xshift=1.5em]uml_component.south) 
    -- 
    node[pos=0, below left] {$uclass$} 
    ++(0,-2em) 
    -| 
    node[pos=0.25, below, align=center] {$\mathvariable{ucomp.name} = \mathvariable{pcomp.name}$}
    %node[pos=1, below left] {$pcomp$} 
...
\draw[consistency relation] ([xshift=-1.5em]java_class.south) 
    ...
    ++(0,-5.1em) 
    -| 
    node[pos=0.25, below, align=center] {$\mathvariable{jclass.packageName} = \mathvariable{pcomp.name}$ \\ $\mathvariable{jclass.name} = \mathvariable{pcomp.name} + \text{\enquote{Impl}}$} 
    node[pos=1, below left] {$pcomp$} 
([xshift=1.6em]pcm_component.south);

\node[code node, below=15.3em of java.north west, anchor=north west] (commonalities_code) {
    \begin{lstlisting}[language=commonalities, frame=none, numbers=none, linewidth=13.3em, basicstyle=\footnotesize\ttfamily]
...
\node[code node, below left=15.3em and 2em of pcm.north east, anchor=north east] (qvt_code1) {
    \begin{lstlisting}[language=QVT, frame=none, numbers=none, linewidth=15.3em, basicstyle=\footnotesize\ttfamily]
...
\node[code node, above right=1.3em and 1em of qvt_code1.south west, anchor=north west] (qvt_code2) {
    \begin{lstlisting}[language=QVT, frame=none, numbers=none, linewidth=14.5em, basicstyle=\footnotesize\ttfamily]
...
\node[above=0.2em of qvt_code1, font=\bfseries\small] {\gls{QVTR} Specification};
