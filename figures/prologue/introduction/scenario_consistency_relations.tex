...
\draw[consistency relation, -latex] (requires_crosspoint|-java_field) -- ++(0,-0.4*\vdistance) -| (java_parameter);
\draw[consistency relation] (pcm_interface) -| (java_interface);


\node[above=0.5em of java_classifier.north, anchor=south, font=\bfseries] (java_label) {\gls{UML} / Java};
\node[anchor=south, font=\bfseries] at (pcm_component.north|-java_label.south) {\gls{PCM}};
\end{tikzpicture}