...
\begin{tikzpicture}[
    ...
    uses/.style={consistency execution, font=\sffamily\footnotesize},
    uses arrow/.style={uses, -latex},
...
\node[below=0.5em of system.north, anchor=north, font=\bfseries] {(Software) System Description};
...
\node[below left=0.66*\vdistance and 0.5*\hdistance of system.center, tool] (uml) {\includegraphics[height=\imagewidth]{figures/images/uml-logo.pdf} \\ \gls{UML}};
...
\draw[knows arrow] ([yshift=-0.8em]software_architect.east) to[bend right=25] node[knows, below right=0em and 0.5em] {\knowsabouttext} ++(0.7*\hdistance, -0.2*\vdistance) to[bend right=40] (umljava.south west);
...
\draw[knows arrow] ([yshift=-0.8em]requirements_engineer.west) to[bend left=15] node[knows, pos=0.5, below left=0.2em and 0.5em] {\knowsabouttext} ++(-0.7*\hdistance, -0.2*\vdistance) to[bend left=22] (umldoors.south);
