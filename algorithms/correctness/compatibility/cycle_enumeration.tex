...
\Procedure{\function{EnumerateFoundCycles}}{$\text{Dual}\ \mathcal{M}^{*}, \mathvariable{pred} \in V_{\mathcal{M}^{*}}$}
    ...
    \For{$base \in \setted{\mathvariable{base}_2, \dots, \mathvariable{base}_n}$}
        ...
        \For{$\mathvariable{cand} \in \mathvariable{currentCycles} \cup \setted{\mathvariable{base}}$}\label{lst:line:newCycles}
            \If{$\mathvariable{pred} \in \mathvariable{cand} \land \function{ReplaceHyperedge}(\mathvariable{pred}, \mathvariable{cand})$}
