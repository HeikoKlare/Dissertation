...
\begin{algorithmic}[1]
\Procedure{\function{EnumerateFoundCycles}}{$\text{Dual}\ \mathcal{M}^{*}, $p$ \in V_{\mathcal{M}^{*}}$}
    \State $\{base_1,\dots,base_n\}$ $\leftarrow$ \function{PatonAlgorithm}($\mathcal{M}^{*}$)
    ...
    \For{$base \in \{base_2, \dots, base_n\}$}
        ...
        \For{$cand \in currentCycles \cup \{base\}$}\label{lst:line:newCycles}
            \If{$p \in cand$ \textbf{and} \function{ReplaceHyperedge}$(p, cand)$}
