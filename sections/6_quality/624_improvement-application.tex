...
To improve such a combination of specifications, virtualization concepts as known from \gls{OSM}~\cite{atkinson2010a} (see \autoref{chap:foundations:multiview:osm}) and the \vitruv approach~\owncite{klare2020Vitruv-JSS} (see \autoref{chap:foundations:multiview:vitruv}) can be applied.
...
In addition, views of a \vsum by now actually assume that only one of them is changed at a time~\owncite{klare2020Vitruv-JSS}, as a developer is supposed to work on one specific view at a time.
