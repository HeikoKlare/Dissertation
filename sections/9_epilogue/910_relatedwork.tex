...
\mnote{Work on goal and methodology}
...
Parts of the discussions in this chapter have been published in previously published work~\owncite{klare2018docsym, klare2019icmt, klare2019models, klare2020compatibility-report, klare2020Vitruv-JSS}.
...
Lenses also extended to the multiary case, in which more than two models need to be kept consistent~\cite{diskin2018MultiModelSynchronization-FASE}.
...
Different ways of how to achieve a \gls{SUM}, i.e., an underlying repository of consistent information~\owncite{meier2019modelsward,meier2020ccis}.
...
Idea from early 2000s ~\cite{vangheluwe2002mpm-AIS, vangheluwe2003mpm-WSC}
...
\textcite{stevens2018bidirectionality-ECMFA} emphasizes importance of bidirectionality for model transformations and software engineering in general.
Bidirectional by direct specification of both directions, such as \gls{QVTO}~\cite{qvt}, specification of one direction and inference of the opposite direction~\cite{xiong2007backwardTransformation-ASE, hettel2008synchronization-ICMT, semerath2016backwardTransformation-MODELS}, or declarative specification of what has to hold and inference of the way to to preserve it (cite limitations of that)
...
\mnote{Conclusion}
Only for some of the discussed tools extensions to support consistency preservation between more than two models have been proposed, which we will discuss subsequently.
...
For transformation networks of arbitrary topology, synchronizing transformations as defined in \autoref{def:synchronizingtransformation} are necessary.
In our definition, this covers transformations that consider changes to both models and are able to update both of them.
While \emph{concurrent synchronization} always considers this scenarios, the term \emph{model synchronization} is used ambiguously for incremental update only~\cite{giese2009incrementalModelSynchronization-SoSym} as well as for concurrent synchronization~\cite{samimi-dehkordi2015bidirectionalSynchronization-ICCKE}.
...
They do, however, leave it up to the developer to implement them and especially consider the case of conflicting user changes, which we have excluded in this thesis and which is not relevant during the execution of a transformation network if transformations are not conflicting, thus making the necessary solution we have proposed in \autoref{chap:synchronization} simpler.

\mnote{TGG approaches}
Approaches for handling concurrent modifications to both models are often concerned with the conflict case, i.e., that changes to both models are conflicting.
...
\textcite{orejas2020IncrementalConcurrentSynchronization-FASE} especially provide different solutions to the synchronization of concurrent modifications to leave it up to the developer how conflicts shall be resolved.
While this may be beneficial behavior for conflicting user changes, having multiple transformation results is not applicable in transformation network as the execution has to proceed with a single one.
...
It should, however, not replace the approach we have presented for the synchronization case in transformation networks, as performing the matching of existing elements by construction of the transformation makes it successful determistically rather than potentially getting unexpected results when considering the scenario as an optimization problem solved by integer linear programming.
...
Second, their approach does not iteratively apply the preservation rules in both directions to improve partial consistency but assume to achieve consistency after executing both, thus they do not reflect that changes to one model may require both models to be changed.
Merging the changes rather than sequentially applying them has the benefit of not requiring the transformation developer to ensure that elements are not created multiple times. However, the merger must correctly consider that case, which, in general, can only be a heuristics.
The differences between our and the discussed approach especially arise from a different approach goal: While we synchronize concurrent changes performed by transformations that should not be conflicting if transformations are not contradictory, their approach merges the user changes as they, of course, can easily be conflicting and these conflicts need to be resolved.
...
\textcite{laemmel2004coupledTransformations-WSET} has already emphasized the necessity to couple transformation at the beginning of research regarding model transformations in \gls{MDSD}.
Combining transformations to networks is a task external to the individual transformations and languages to define them, which is why existing transformation languages to not consider the combination of transformations developed with them.
...
She also derives the relaxed notion of \emph{binary-implemented} relations, which only requires models consistent to the binary relations to also be consistent to the multiary one, but not vice versa.
...
A multi-modeling platform~\cite{denton2008naomi-Models} considers change propagation based on a dependency graph between the models and performs a depth-first search.
...
\mnote{Provenance}
Provenance is also a topic of growing attention and importance in research for bidirectional transformations~\cite{cleve2019dagstuhl,anjorin2019provenance-tapp}.
While \textcite{anjorin2019provenance-tapp} especially consider provenance information about changes performed by a single transformations, we provide such information for the cause of a transformation network to fail.
This affects the network developers rather than the user of transformations, who tries to understand what happens.
...
Although these work considers quality properties of transformations such as reuse, which we discuss in \autoref{chap:classification}, they are not concerned with quality properties of a transformation network and especially the reuse of transformations in other network.
...
These approaches deal with specific problems of execution of and compatibility in transformation networks and are thus related to our work on compatibility and orchestration, presented in \autoref{chap:compatibility} and \autoref{chap:orchestration}.
...
Finally, factorization approaches identify common parts of transformations and extract them into a base transformation from which the individual parts are extended~\cite{cuadrado2008a}.
...
In contrast to our work, these approaches support the specification on multiary relations between multiple, i.e., more than two, metamodels.
...
\mnote{Validation and verification of OCL constraints}
There are several approaches for the validation of OCL constraints used to define conditions on valid models or to define model transformations.
