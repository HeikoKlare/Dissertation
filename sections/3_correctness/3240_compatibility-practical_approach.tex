...
To find a redundant relation, the procedure identifies an alternative concatenation of consistency relations relating the same \metaclasses, according to \autoref{def:leftequalredundancy}, and performs a \emph{redundancy test} with respect to that alternative concatenation.
...
Developers, however, use intensional specifications of the constraints that have to hold within transformations.
...
The consistency relation $\consistencyrelation{CR}{PE}$ of the running example relates an employee's name to the concatenation of first and last name of a person and thus contains three properties.
...
As discussed before, this allows us to plug in different implementations for the redundancy test, of which we will depict one in the subsequent subsection.
...
The algorithm is able to prove compatibility by removing redundant relations until independent trees remain.
