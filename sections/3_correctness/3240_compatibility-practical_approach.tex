...
We have yet introduced the redundancy test of predicates in the decomposition procedure as a black box, which can be realized by any approach that is able to prove redundancy of predicates, to foster independent reuse of the proposed decomposition procedure and the redundancy test to be presented in the following.
...
\begin{align*}
    &
        [(\propdisplay{\small Person::firstname} = \mathvariable{fstn1}) \land (\propdisplay{\small Person::lastname} = \mathvariable{lstn1})\\
    &
        \formulaskip \land (\propdisplay{\small Resident::name} = \mathvariable{fstn1} + \text{\enquote{\textvisiblespace}\textvisiblespace}} + \mathvariable{lstn1}) \\%\right]\\
    & 
        \land %\left[
            (\propdisplay{\small Person::firstname} = \mathvariable{fstn2}) \land (\propdisplay{\small Person::lastname} = \mathvariable{lstn2})\\
    &
        \formulaskip \land (\propdisplay{\small Employee::name} = \mathvariable{fstn2} + \text{\enquote{\textvisiblespace}\textvisiblespace}} + \mathvariable{lstn2})]\\
    & 
        \Rightarrow
        \left(\exists n : (\propdisplay{\small Resident::name} = \mathvariable{n}) \land (\propdisplay{\small Employee::name} = \mathvariable{n})\right)
\end{align*}
