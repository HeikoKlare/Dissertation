...
Derive research questions from the formalization (compatibility, sychronization, orchestration, probability of errors)
}

\section{Notions of Consistency and its Preservation}

\subsection{Intensional vs. Extensional Specification}
...
\subsection{Modular vs. Monolithic Consistency Specification}
...
\subsubsection{A Monolithic Notion of Consistency}
...
\subsubsection{A Modular Notion of Consistency}
...
\subsection{Consistency and its Preservation}

Discuss how consistency is preserved based on a notion of consistency (relations)
...
\begin{description}
    \item[Consistency Preservation Rules:] We need binary rules that restore consistency after a model of a pair was modified. These are the binary, modular transformations of the networks
    \item[Orchestration Function:] We need a function that is able to decide in which order transformation have to be executed after a change.
    \item[Application Function:] We need a function that is able to apply the rules in the order delivered by the orchestration function. 
\end{description}
...
The process is depicted in \autoref{fig:formal:executionprocess}. Given models to are consistent according to some consistency relations and changes to them, the orchestration delivers an order of consistency preservation rules (CPRs) for that networks, which is used to parametrize the application function that executes these rules in the given order.
...
\todo{Monolithic notion can be seen as a degraded transformation network only having one transformation, thus no orchestration, not incompatibility.
Finally, even with multiary transformation, we want to be able to combine them to networks}
