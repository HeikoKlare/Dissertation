...
The benefits of this approach are twofold. First, there is no additional logic required for unifying the changes performed by indepedently executed \glspl{consistency preservation rule}. 
Second, the unification may deliver a model that is not consistent ot any of the \glspl{consistency relation} anymore, whereas consecutive execution at least gives the guarantee that the models are consistent to the last applied \gls{consistency preservation rule}.
...
Identifying an appropriate notion of \emph{compatibility} and how to check it constitutes a central research question of this work and will be discussed in \autoref{sec:correctness:compatibility}.
...
\begin{definition}[Correctness of \ModelLevelConsistencyPreservationRule]
...
\begin{definition}[Synchronizing Transformation]
    ...
    A synchronizing transformation is a pair $\synchronizingtransformation = \tupled{\consistencyrelation{CR}{}, \consistencypreservationrule{\consistencyrelation{CR}{}}}$.
\end{definition}



\todo{GO ON HERE: Start section for orchestration, finally define transformation network as tuple and derive research questions from that
First add reference to correctness notion by stevens for rule above!}
