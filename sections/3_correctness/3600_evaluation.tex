...
In the preceding chapters, we have discussed several aspects of a well-defined notion of consistency and correctness of its preservation in transformation networks.
Based on the assumptions we made, we were able to prove several statements regarding decidability of problems and the effects of approaches we proposed, such as the analysis of compatibility or the construction of synchronizing transformations.
...
These especially concern the applicability of our approaches and also, at least implicitly, the appropriateness of our formalism.
...
For each of the topics, we provide a plan according to the \gls{GQM} approach, for which the original idea was presented by \textcite{basili1984GQM-TSE}.
We define goals that we want to achieve with our evaluation, derive questions that we answer to identify whether we reached the underlying goal, and define metrics whose results we use to get a quantitative measure for answering the questions.
...
\todo{Make unified table for each GQM plan}
...
\begin{table}
    \begin{tabular}{p{8em} p{20em}}
        ...
        \metric & \metrictext{Precision: Ratio of true positives to true positives + false positives (false positives have to be 0)} \\\addlinespace[\outerrowspacing]
        \question{Applicability%Usefuleness/Conservativeness
        } & How often does the analysis not prove compatibility although it is given? (how conservative is it?) \\\addlinespace[\innerrowspacing]
        \metric & \metrictext{Recall: Ratio of true positives to true positives + false negatives (false negatives indicate conservative case)}\\
        \bottomrule
    \end{tabular}
...
\begin{table}
    \begin{tabular}{p{8em} p{20em}}
        \toprule
        \goal{Categorization} & Show that the relations between mistakes, faults and failures in the categorization for transformation network errors are correct. \\\addlinespace[\innerrowspacing]
        ...
        \question{Correctness} & Does the application of synchronization techniques lead to correct synchronizing transformations? \\\addlinespace[\innerrowspacing]
        \metric & \metrictext{Ratio of changes that are propagated correctly after applying the techniques to those that are not propagated correctly} \\\addlinespace[\outerrowspacing]
        \question{Relevance} & Does the application of synchronization techniques resolve any incorrectnesses? \\\addlinespace[\innerrowspacing]
        \metric & \metrictext{Ratio of changes that are propagated correctly after applying the techniques to those that were propagated correctly before}\\
        \bottomrule
    \end{tabular}
    \caption[Goals, questions and metrics for errors and synchronization]{Goals, questions and metrics for errors and synchronization evaluation}
...
Threats:
- The transformation were not constructed synchronizing, thus obviously many synchronization errors occur. If the scenario is foreseen, there may be much less errors at this level and more at the others -> need to perform further evaluation were developer know about necessity to use transformations in a network to properly construct them and then see how often that still fails.
