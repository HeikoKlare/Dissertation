...
When we consider a tiüöe of models, we would intuitively say that it is consistent if it fulfills some kind of constraints.
Defining these constraints to derive or check whether a given tuple of models is consistent constitutes an \emph{intensional specification} of consistency, because the tuple that contains all consistent models is intensionally represented by these constraints and can be derived from it.
We can consider a set of constraints as a predicate, i.e. a Boolean-valued function $I$, which indicates whether a model tuple $\modeltuple{m}$ fulfills the constraints $I: \metamodeltupleinstanceset{M} \mapsto \setted{true, false}$. Then we can say that:
\begin{align*}
    \modeltuple{m} \consistenttomath I \equivalentperdefinition I(\modeltuple{m}) = true
\end{align*}
On the contrary, one can also enumerate the consistent tuples of models, thus a tuple of models is considered consistent if it is contained in that enumeration.
This constitutes an \emph{extensional specification} of consistency.
Given such an enumeration $E = \setted{\modeltuple{m} \mid \modeltuple{m} \mathtext{is consistent}}$, we can say that:
\begin{align*}
...
\begin{align*}
    E = \setted{\modeltuple{m} \mid I(\modeltuple{m}) = true}
\end{align*}
...
\begin{align*}
    I(\modeltuple{m}) \mapsto 
    \begin{cases} 
        true, & \modeltuple{m} \in E\\
        false, & \modeltuple{m} \not\in E\\
    \end{cases}
\end{align*}
...
Like discussed for intensional and extensional consistency specification, this can be expressed by a tuple of models fulfilling constraints or being contained in a relation.
...
\end{align*}
We would require from that function that: $\forall \modeltuple{m} : \function{Cp}(\modeltuple{m}) \mathtext{is consistent}$.
...
For example, given a monolithic relation $\consistencyrelation{R}{}$, $\function{Cp}$ is supposed to fulfill that: $\forall \modeltuple{m} : \function{Cp}(\modeltuple{m}) \in \consistencyrelation{R}{}$.
...
Consider the input models $\tupled{\model{m}{1}, \model{m}{2}m, \model{m}{3}}$ that are not consistent to $\consistencyrelation{R}{1,2}$ and $\consistencyrelation{R}{2,3}$.
Now if we apply the functions independently, we have $\function{Cp}_{1,2}(\tupled{\model{m}{1}, \model{m}{2}}) = \tupled{\model{m'}{1}, \model{m'}{2}} \in \consistencyrelation{R}{1,2}$ and 
$\function{Cp}_{2,3}(\tupled{\model{m}{2}, \model{m}{3}}) = \tupled{\model{m''}{2}, \model{m''}{3}} \in \consistencyrelation{R}{2,3}$.
...
If we consecutively apply the two given functions, we know that $\function{Cp}_{1,2}(\tupled{\model{m}{1}, \model{m}{2}}) = \tupled{\model{m'}{1}, \model{m'}{2}} \in \consistencyrelation{R}{1,2}$ and 
$\function{Cp}_{2,3}(\tupled{\model{m'}{2}, \model{m}{3}}) = \tupled{\model{m''}{2}, \model{m''}{3}} \in \consistencyrelation{R}{2,3}$.
...
In fact, we need some method to decide in which order and how often the \glspl{consistency preservation rule} are applied to result in a consistent tuple of models.
...
Given a function $\function{Cp}$, the relation is implied by its fixed-points: $\consistencyrelation{R}{} = \setted{\tupled{m} \mid \function{Cp}(\tupled{m}) = \tupled{m}}$.
...
\begin{properdescription}
    ...
    \item[\Glspl{consistency preservation rule}:] Functions that that restore consistency for a pair (or in general tuple) of models after they were modified to an inconsistent state.
