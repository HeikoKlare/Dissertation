...
\todo{Add an adapted version of the following to fine-grained consistency notion for trace models:
\mnote{Explicit trace models}
One might argue that consistency is usually traced by means of a \emph{trace} or \emph{correspondence model}, which stores the pairs of element tuples in models that fulfill a consistency relation.
While the realization of the \vitruv approach contains an explicit correspondence model, we do not explicitly consider it in this formalism due to two reasons.
...
However, without loss of generality we can assume such identifying information in a formalism, like it is usually done in formalisms of transformations such as the one used in \cite{stevens2010a}.
Second, a trace model can, from a theoretical perspective, be treated as a usual model, thus always defining consistency between one concrete and one correspondence model. This conforms to the fact that each n-ary relation can be expressed by binary relations to an additional model (in this case the trace model), as discussed in \cite{stevens2020largesosym, cleve2019dagstuhl}.
}
