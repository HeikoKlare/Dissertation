...
In addition to the above considerations, a transformation may be prevented from being partial-consistency-improving by the addition or removal of a condition element to improve consistency affects further condition elements.
...
\begin{properdescription}
    ...
    Thus, the consistency preservation rule must only consider that the corresponding element may have been removed, but must do nothing.
    If the corresponding element was changed, which is identified by the trace model that still contains a link to a changed element, it must be adapted such that both elements form a consistency relation pair again.
...
\mnote{Trace links}
In \owncite{klare2019icmt}, we have proposed a strategy to identify such corresponding elements.
...
\textcite{kramer2017a} proposes feature models that express all kinds of possible changes in these types of models.
...
\begin{properdescription}
    ...
    \item[Subtractive root change (possibly delete):] The removal of a root element can lead to the removal of a condition element, which involves the root element. Since it completely removes an element, it can neither lead to a change or addition of a condition element.
