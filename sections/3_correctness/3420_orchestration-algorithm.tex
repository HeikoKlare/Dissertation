    ...
    \begin{algorithmic}[1]
        \Procedure{$\function{Apply}_{\transformationset{T}}$}{$\modeltuple{m}, \changetuple{\metamodeltuple{M}}$}
            ...
            \While{$\transformation{t}_{next} \neq \bot$}
                \State $(\modeltuple{m}, \changetuple{\metamodeltuple{M}})$ $\leftarrow$ $\function{Gen}_{\metamodeltuple{M}, \transformation{t}_{next}}(\modeltuple{m}, \changetuple{\metamodeltuple{M}})$ \label{algo:orchestration:application:line:stepcalculation}
...
Next to \function{Orchestrate}, the algorithm uses the external functions \function{Gen} and \function{CheckConsistency}.
...
We have discussed that non-termination of the algorithm can occur because no orchestration that yields consistent models exists at all or because the algorithm is not able to find it.
