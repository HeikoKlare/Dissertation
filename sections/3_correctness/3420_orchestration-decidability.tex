...
\todo{Orchestrate must ensure that the returned transformation can be applied (may not be defined). Only the the transformation can be applied afterwards by the gen function.}
...
\begin{proof}
	...
	\begin{enumerate} %[1., left=0pt .. \parindent]
        \item 
...
\end{corollary}
\begin{proof}
    According to \autoref{def:applicationfunctionsolveorchestrationproblem} an optimal application function can only be defined if a solution for the orchestration and orchestration existence problem, respectively, exists.
