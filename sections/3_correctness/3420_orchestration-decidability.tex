...
The \function{Orchestrate} function may implement different strategies for selecting the next transformation, which we will later discuss in more detail.
...
\begin{proof}
    ...
    Then for any orchestration function $\orcfunction{\transformationset{T}_\mathvariable{inc}}$ that delivers a consistent orchestration for these inputs, we know that $\abs{\orcfunction{\transformationset{T}_\mathvariable{inc}}(\modeltuple{m}, \changetuple{})} >= x-1 = n+1 > n$.
...
\mnote{No upper bound for deciding orchestration existance}
...
This behavior is actually desired, thus prevent it by requiring confluence is not practical.
