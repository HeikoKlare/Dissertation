...
\begin{algorithm}
   % \input{algorithms/correctness/synchronization/find_corresponding_elements.tex}
    \begin{algorithmic}[1]
        \Procedure{$\function{Apply}_{\transformationset{T}}$}{$\modeltuple{m} = \tupled{\model{m}{1}, \dots, \model{m}{n}}, \changetuple{\metamodeltuple{M}} = \tupled{\change{\metamodel{M}{1}}, \dots, \change{\metamodel{M}{n}}}$}
...
An application function according to \autoref{def:applicationfunction} is parametrized by the set of transformations $\transformationset{T}$ it is supposed to be executed on and a transformation network according to \autoref{def:transformationnetwork} is defined to consistent of a set of transformations and an application function, which may suggest to define this function specific for one network.
\autoref{algo:orchestration:application} is, however, independent from the concrete network and is able to process any set of transformations.
It thus realizes not one application function, but actually a family of application functions for all possible transformation sets $\transformationset{T}$.
...
\todo{CheckConsistency can also be implemented by executing all transformation and checking whether anything changed, as long as the transformations are hippocratic. Then it is not necessary to have the consistency relations explicitly, but applying the consistency preservation rules is enough.}
...
\begin{theorem}[Upper Bound of Shortest Consistent Orchestration]
...
\begin{align*}
    &
    \model{m}{i}' = \tupled{\mathvariable{time}_{\model{m}{i}'}, \mathvariable{cont}_{\model{m}{i}'}, \mathvariable{pos}_{\model{m}{i}'}} \equalsperdefinition \changetuple{\metamodel{M}{i,\TuringMachine}}(\model{m}{i}) \\
    &
    \model{m}{k}' = \tupled{\mathvariable{time}_{\model{m}{k}'}, \mathvariable{cont}_{\model{m}{k}'}, \mathvariable{pos}_{\model{m}{k}'}} \equalsperdefinition \changetuple{\metamodel{M}{k,\TuringMachine}}(\model{m}{k}) \\
    &
    ...
    \begin{cases}
        ...
        \formulaskip\formulaskip\formulaskip
            \land \exists \tupled{\mathvariable{cont}_{\model{m}{k}'}[\mathvariable{pos}_{\model{m}{k}'}], \mathvariable{dir},\mathvariable{repl}} \in \Tr(Q(\metamodel{M}{i,\TuringMachine}),Q(\metamodel{M}{k,\TuringMachine}))\\[.4em]
    ...
    \begin{cases}
        ...
        \formulaskip\formulaskip\formulaskip
            \land \exists \tupled{\mathvariable{cont}_{\model{m}{i}'}[\mathvariable{pos}_{\model{m}{i}'}], \mathvariable{dir},\mathvariable{repl}} \in \Tr(Q(\metamodel{M}{i,\TuringMachine}),Q(\metamodel{M}{k,\TuringMachine}))\\[.4em]
...
% \end{multline*}
% }
where $\mathvariable{cont}|_{\mathvariable{pos} \gets \mathvariable{repl}} \equalsperdefinition \mathvariable{cont}[0\:..\:\mathvariable{pos}-1] \cdot \mathvariable{repl} \cdot \mathvariable{cont}[\mathvariable{pos}+1\:..\:|\mathvariable{cont}|-1]$.
...
\begin{lemma}[Transformation Network from Turing Machine]
	...
	If executing the transformations terminates with the final changes $\changetuple{\metamodeltuple{M},f}$, then the model in $\modeltuple{m}_{f} = \tupled{\change{1,\metamodeltuple{M},f}(\model{m}{1,\TuringMachine,x}), \dots, \change{n,\metamodeltuple{M},f}(\model{m}{n,\TuringMachine, x})}$ with the highest timestamp contains $\TuringMachine(x)$ as tape content.
\end{lemma}
\begin{proof}
...
\begin{corollary}[Application Function Optimality]
...
\begin{corollary}
    \function{Apply} according to \autoref{algo:orchestration:application} cannot always terminate and return consistent models whenever an orchestration exists that yields them for every possible input.
\end{corollary}
\begin{proof}
