...
In addition, they usually assume the input models to be consistent and then ensure that after applying the input and the output changes to the models, the resulting models are consistent again.
Since such consistency preservation rules are defined for consistent input models, their behavior in case of inconsistent models is undefined, either returning $\bot$ or a change that does not necessarily guarantee that the models are consistent after applying the input and output changes.
...
In general, unidirectional consistency preservation rules need to be partial, because there can be models for which no other models can be generated such that they are consistent to a set of consistency relations.
...
A unidirectional consistency preservation rule does usually not appear alone but in combination with another rule for the opposite direction.
...
We call such a bidirectional transformation correct if both consistency preservation rules are correct according to \autoref{def:unidirectionalconsistencypreservationrulecorrectness}. %, i.e., they both preserve consistency according to the underlying consistency relation set.
