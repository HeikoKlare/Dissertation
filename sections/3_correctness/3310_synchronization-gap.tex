...
Since they are defined for consistent input models, their behavior in case of inconsistent models is undefined, either returning $\bot$ or a change that does not necessarily guarantee that the models are consistent after applying the input and output changes.
...
This was only an optional requirement for synchronizing consistency preservation rules defined in \autoref{def:consistencypreservationrule}, because there may be changes to both models which cannot be processed reasonably as one the changes may need to be reverted to achieve consistency.
...
\mnote{Mandatory partiality of unidirectinoal preservation rules}
In general, unidirectional consistency preservation rules need to be partial, because there can be models for which no other models can be generated such that they are consistent with respect to a set of consistency relations.
