...
\end{theorem}
\begin{proof}
	After the recursive call in \autoref{algo:orchestration:provenance:line:recursive_call}, the model tuple yielded by applying the current changes $\changetuple{\metamodeltuple{M},\mathvariable{candidate}}$ to the initial model tuple $\modeltuple{m}$ is consistent to all executed transformations in $\provalgexecuted$ according to \autoref{theorem:provenance_correctness}.
\end{proof}	
