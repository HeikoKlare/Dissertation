...
\begin{algorithm}
    ...
    \Procedure{\function{Propagate}}{$\transformationset{T}, \modeltuple{m} = \tupled{\model{m}{1}, \dots, \model{m}{n}}, \changetuple{\metamodeltuple{M}} = \tupled{\change{\metamodel{M}{1}}, \dots, \change{\metamodel{M}{n}}}$}
        ...
        \For{$\provalgcandidate \in \transformationset{T} \setminus \provalgexecuted \mid \changetuple{\metamodeltuple{M}}.\mathvariable{affects}(\provalgcandidate)$} \label{algo:orchestration:provenance:line:loop_start}
            ...
            \If{$\neg \mathvariable{isConsistent}$} \label{algo:orchestration:provenance:line:failcheck}
...
\todo{Adapt because of returning bot after generalization function and after recursive call}
...
\subsection{Correctness, Termination and Design Goals}
...
Whenever the \function{Propagate} function fails and returns $\bot$, we know that for the current transformations in $\provalgexecuted$ an orchestration exists that yields the current changes in $\changetuple{\metamodeltuple{M}} = \tupled{\change{\metamodel{M}{1}}, \dots, \change{\metamodel{M}{n}}}$, for which we know that applied to the original models the result $\tupled{\change{\metamodel{M}{1}}(\model{m}{1}), \dots, \change{\metamodel{M}{n}}(\model{m}{n})}$ is at least consistent to $\provalgexecuted$.
