...
The execution of a bidirectional transformation then consists of the consecutive application of execution steps until the delivered models are consistent.
...
\begin{algorithm}
    \begin{algorithmic}[1]
        \Procedure{\textsc{Execute-Bidirectional-Transformation}}{$\transformation{T} = \tupled{\consistencyrelationset{CR}, \consistencypreservationrule{\consistencyrelationset{CR}}^{\rightarrow}, \consistencypreservationrule{\consistencyrelationset{CR}}^{\leftarrow}}, \model{m}{1}, \model{m}{2}, \change{\metamodel{M}{1}}$}
            ...
            \While{$\neg \tupled{\change{\metamodel{M}{1}}(\model{m}{1}), \model{m}{2}} \consistenttomath \consistencyrelationset{CR}$}
                ...
                \If{$executionResult = \bot$}
                    \State \Return{$\bot$} \label{algo:executebidirectionaltransformation:line:returnbot}
                \Else
                    \State ($\model{m}{1}, \model{m}{2}, \change{\metamodel{M}{1}}$) $\equalsperdefinition$ $executionResult$
                \EndIf
            \EndWhile
            \State \Return $\tupled{\change{\metamodel{M}{1}}(\model{m}{1}), \model{m}{2}}$ \label{algo:executebidirectionaltransformation:line:returnresult}
        \EndProcedure
    \end{algorithmic}
    \caption[Algorithm to execute a bidirectional transformation]{Algorithm to execute a bidirectional transformation.}
    \label{algo:executebidirectionaltransformation}
\end{algorithm}
...
\begin{theorem}{Bidirectional Transformation Execution Consistency}
    If \autoref{algo:executebidirectionaltransformation} terminates, it either returns $\bot$ or a consistent model pair.
\end{theorem}
\begin{proof}
    \autoref{algo:executebidirectionaltransformation} can terminate by one of its two return statements in \autoref{algo:executebidirectionaltransformation:line:returnbot} and \autoref{algo:executebidirectionaltransformation:line:returnresult}.
    \autoref{algo:executebidirectionaltransformation:line:returnbot} returns $\bot$, which fulfills the theorem statement.
    \autoref{algo:executebidirectionaltransformation:line:returnresult} returns the model pair $\tupled{\change{\metamodel{M}{1}}(\model{m}{1}), \model{m}{2}}$.
    ...
    Since the negation of the loop condition is $\tupled{\change{\metamodel{M}{1}}(\model{m}{1}), \model{m}{2}} \consistenttomath \consistencyrelationset{CR}$, the result fulfills the theorem statement.
\end{proof}
...
\begin{theorem}[Termination of Partial-Consistency-Improving Bidirectional Transformation Execution]
    \label{theorem:bidirectionaltransformationtermination}
    ...
    Then \autoref{algo:executebidirectionaltransformation} does always terminate.
\end{theorem}
\begin{proof}
    ...
    \begin{align*}
        &
        \mathvariable{sizeDifference}_{i} = \mathvariable{sizeDifference}_{0} - \sum_{k=1}^{i} \mathvariable{sizeDifferenceReduction}_{k} \\
        & \formulaskip
        \leq \mathvariable{sizeDifference}_{0} - \sum_{k=1}^{i} 1 = \mathvariable{sizeDifference}_{0} - i
    \end{align*}
    This implies that:
    \begin{align*}
        i > \mathvariable{sizeDifference}_{0} \Rightarrow \mathvariable{sizeDifference}_{i} <= 0
    \end{align*}
...
\begin{definition}[Synchronizing Bidirectional Transformation Execution Step]
    Let $\transformation{T} = \tupled{\consistencyrelationset{CR}, \consistencypreservationrule{\consistencyrelationset{CR}}^{\rightarrow}, \consistencypreservationrule{\consistencyrelationset{CR}}^{\leftarrow}}$ be a bidirectional transformation for metamodels $\metamodel{M}{1}$ and $\metamodel{M}{2}$.
    An \emph{synchronizing execution step} $\function{SyncEx}_{\transformation{T}}^1$ of $\transformation{T}$ is a function:
    \begin{align*}
...
Step integrates both changes, comparare to ordinary step, but first treats changes to m2 as already applied, i.e., given models are inconsistent, then processes the changes in the opposite direction to generate necessary changes to restore consistency from them.

\begin{algorithm}
    \begin{algorithmic}[1]
        \Procedure{\textsc{Execute-Synchronizing-Bidirectional-Transformation}}{$\transformation{T} = \tupled{\consistencyrelationset{CR}, \consistencypreservationrule{\consistencyrelationset{CR}}^{\rightarrow}, \consistencypreservationrule{\consistencyrelationset{CR}}^{\leftarrow}}, \model{m}{1}, \model{m}{2}, \change{\metamodel{M}{1}}, \change{\metamodel{M}{2}}$}
            \If{$\neg \tupled{\model{m}{1}, \model{m}{2}} \consistenttomath \consistencyrelationset{CR}$}
                \State \Return{$bot$}
            \EndIf
            \If{$\neg \tupled{\change{\metamodel{M}{1}}(\model{m}{1}), \model{m}{2}} \consistenttomath \consistencyrelationset{CR}$}
                ...
                \If{$executionResult = \bot$}
                    \State \Return{$\bot$} \label{algo:executesynchronizingbidirectionaltransformation:line:returnbot1}
                \Else
                    \State ($\model{m}{1}, 
            ...
            \While{$\neg \tupled{\change{\metamodel{M}{1}}(\model{m}{1}), \model{m}{2}} \consistenttomath \consistencyrelationset{CR}$}
                ...
                \EndIf
            \EndWhile
            \State \Return $\tupled{\change{\metamodel{M}{1}}(\model{m}{1}), \model{m}{2}}$ \label{algo:executesynchronizingbidirectionaltransformation:line:returnresult}
        \EndProcedure
    \end{algorithmic}
    \caption[Algorithm to execute a bidirectional transformation for synchronization]{Algorithm to execute a bidirectional transformation in the synchronization case.}
    \label{algo:executesynchronizingbidirectionaltransformation}
\end{algorithm}
