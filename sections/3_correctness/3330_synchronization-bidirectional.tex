...
\begin{proof}
    ...
    This statement can only be reached if the previous while loop terminates by non-fulfillment of the loop condition.
...
\begin{proof}
    ...
    %\end{align*}
    For reasons of simplicity, we have ignored that $\mathvariable{sizeDifferenceReduction}_{k} = 0\; (k > i)$ as soon as $\mathvariable{sizeDifference}_{i} = 0$, because $\mathvariable{sizeDifference}_{i}$ cannot be less than $0$.
...
It only produces a change in the first model, such that afterwards ordinary transformation execution steps that only need to deal with a change to one model can be applied.
...
This algorithm is an extension of \autoref{algo:synchronization:execute_bidirectional_transformation} for the non-synchronizing case.
...
We have proven that a bidirectional transformation that is partial-consistency-improving can be executed for two given models and changes to both of them such that consistent models are delivered, as long as the transformation is able to process the changes.
