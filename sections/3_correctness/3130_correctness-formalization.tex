...
\todo{Prüfen, ob es valide ist, dass die Funktion ist partiell ist. Man könnte bspw. sagen, die gibt im Fehlerfall einfach die leere Sequenz zurück. Dann müsste halt App prüfen, ob die Modelle schon konistent waren oder nicht.}
...
\begin{definition}[Transformation Network Correctness]
    ...
    We say that $\transformationnetwork{N}$ is \emph{correct} if, and only if, its transformations in $\transformationset{T}$ as well as the application function $\appfunction{\orcfunction{\transformationset{T}}}$ is correct, i.e.
    \begin{align*}
       % & 
