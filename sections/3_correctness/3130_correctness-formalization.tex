...
To precisely define this behavior, we first introduce a notion of \emph{changes} and afterwards of \emph{\glspl{consistency preservation rule}}, which we also refer to as \emph{synchronizing} consistency preservation rules.
...
We already discussed that it is possible to define \glspl{consistency relation} and derive \glspl{consistency preservation rule} from them or to only define the \glspl{consistency preservation rule}, which then imply the \glspl{consistency relation} by their image, i.e., the set of all models that can be derived from applying the consistency preservation rule to any models and changes for which it is defined.
...
In the following, we will only refer to transformations rather than \glspl{consistency relation} and \glspl{consistency preservation rule} if the distinction is not necessary.
...
We define a transformation \gls{application function} that applies transformations to a given tuple of models and changes according to an order delivered by an \gls{orchestration function}.
