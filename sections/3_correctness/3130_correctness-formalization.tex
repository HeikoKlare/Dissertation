...
\begin{definition}[\ModelLevelConsistencyRelation]
    ...
    \end{align*}
    Otherwise, we call $\modeltuple{m} \mathtextspacearound{inconsistent to} \consistencyrelation{CR}{}$.
\end{definition}
...
\begin{properdescription}
    ...
    \item[Consistency assumption (induction):] We assume models to be consistent before a change is processed by consistency preservation rules. Otherwise the preservation rules would need to be able to handle arbitrary states of the models and it would be unclear consistency to which consistency relations has to be restored.
...
Like we already discussed for consistency preservation rules in general, incremental preservation rules can be realized in an either monolithic or modular way.
...
\mnote{Synchronizing consistency preservation rules circumvent drawbacks}
...
\begin{definition}[\ModelLevelConsistencyPreservationRule]
    ...
    Let $\metamodel{M}{1}, \metamodel{M}{2}$ be two metamodels and $\consistencyrelation{CR}{} \subseteq \metamodelinstanceset{M}{1} \times \metamodelinstanceset{M}{2}$ a binary \modellevelconsistencyrelation between them.
    A \emph{\modellevelconsistencypreservationrule} $\consistencypreservationrule{\consistencyrelation{CR}{}}$ for the relation $\consistencyrelation{CR}{}$ is a function:
    \begin{align*}
...
\mnote{Synchronization behavior conforms to existing definitions}
...
\begin{definition}[\ModelLevelConsistencyPreservationRule Correctness]
    ...
    We call $\consistencypreservationrule{\consistencyrelation{CR}{}}$ \emph{correct} if the resulting models when applying the generated changes are consistent to $\consistencyrelation{CR}{}$ again:
    \begin{align*}
        &
        \forall 
...
\begin{definition}[Synchronizing Transformation Correctness]
    ...
    We call a synchronizing transformation $\transformation{t} = \tupled{\consistencyrelation{CR}{}, \consistencypreservationrule{\consistencyrelation{CR}{}}}$ correct if, and only if, $\consistencypreservationrule{\consistencyrelation{CR}{}}$ is correct according to \autoref{def:consistencypreservationrulecorrectness}.
\end{definition}

\mnote{Generalize consistency and its preservation to encapsulating transformations}
...
\begin{definition}[Consistency to Transformation]
    ...
    We say that any tuple of models $\modeltuple{m}$ is \emph{consistent to} $\transformation{t}$ if, any only if, it is consistent to its consistency relation, i.e.
    \begin{align*}
    ...
    \end{align*}
    %
    For a set of transformations $\transformationset{T}$, we say that a model tuple $\modeltuple{m}$ is \emph{consistent to} $\transformationset{T}$ if, any only if, it consistent to all transformations in it, i.e.
    \begin{align*}
...
\begin{definition}[Transformation Orchestration Function] \label{def:orchestrationfunction}
    Let $\transformationset{}$ be a set of transformations with consistency relations and according consistency preservation rules for metamodels $\metamodeltuple{M} = \tupled{\metamodelsequence{M}{n}}$.
    A transformation orchestration function $\orcfunction{\transformationset{T}}$ for these transformations is a function:
    \begin{align*}
        &
    ...
    \end{align*}
    $\transformationset{T}^{< \mathbb{N}}$ denotes all finite sequences of transformations in $\transformationset{T}$, i.e., $\transformationset{T}^{< \mathbb{N}} \equalsperdefinition \emptyset \cup \transformationset{T}^1 \cup \transformationset{T}^2 \cup \dots$.
\end{definition}
...
We will discuss in \autoref{chap:orchestration} in detail whether we can find restrictions for the transformations that ensure that an orchestration that delivers a consistent result always exists.
...
As an alternative, we might require the function to always return consistent models when the orchestration function delivers a sequence of transformations whose application leads to a consistent tuple of models.
...
\begin{definition}[Transformation Application Function Correctness]
    ...
    \begin{align*}
        &
        \appfunction{\orcfunction{\transformationset{T}}} \mathtextspacearound{is correct} \equivalentperdefinition \\
        & \formulaskip
        \forall \modeltuple{m} \in \metamodeltupleinstanceset{M} : \forall \changetuple{\metamodeltuple{M}} = \tupled{\change{\metamodel{M}{1}}, \dots, \change{\metamodel{M}{n}}} \in \changeuniverse{\metamodeltuple{M}} :
        \modeltuple{m} \consistenttomath \consistencyrelationset{CR} \Rightarrow \\
        & \formulaskip
...
Actually, an application function that always return $\bot$ is correct according to that definition.
...
\begin{definition}[Transformation Network Correctness]
    ...
    \begin{align*}
        & 
        \transformationnetwork{N} \mathtextspacearound{is correct} \equivalentperdefinition \\
        & \formulaskip
        \forall \transformation{t} \in \transformationset{T} : \transformation{t} \mathtextspacearound{is correct} \land \appfunction{\orcfunction{\transformationset{T}}} \mathtextspacearound{is correct}
    \end{align*}
\end{definition}
