...
\begin{figure}
    ...
    \caption[Monolithic consistency relation that cannot be modularized]{A monolithic consistency relation that cannot be expressed by binary relations.}
...
\begin{figure}
    ...
    \caption[Unidirectional consistency preservation in networks]{Execution of consistency preservation rules of which at least one cannot be unidirectional, because both models are modified by a user or other consistency preservation rules.}
...
\begin{definition}[Change]
    ...
    \begin{align*}
        \changetuple{\metamodeltuple{M}} = \tupled{\change{\metamodel{M}{1}}, \dots, \change{\metamodel{M}{n}}} \in \changeuniverse{\metamodel{M}{1}} \times \dots \times \changeuniverse{\metamodel{M}{n}} %\change{\metamodel{M}{i}} \in \changeuniverse{\metamodel{M}{i}}
    \end{align*}
...
%\todo{Maybe remove \enquote{model-level} if we do not need a fine-grained notion of preservation rules. Maybe better call it \enquote{synchronizing} to be able to distinguish it from non-synchronizing ones later. -- No, not synchronizing, because other notion is only relevant in synchronization chapter, nowhere else.}
...
%Discuss that we do not define for which inputs the function may be undefined. It is up to the developer to reasonably use the ability to specify the function partially. In the extreme case, the function could be undefined for all inputs, which does not make sense, but that is up to the developer.}
...
We also use the short term \emph{transformation} to refer to a synchronizing transformation.
