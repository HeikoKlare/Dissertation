...
\todo{In addition, approaches like the one presented in \autoref{stevens2020BidirectionalTransformationLarge-SoSym} use a notion of \emph{authoritative} models, which specify the models that may not be changed, e.g., because they are immutable or because they were originally changed by the user and reverting those changes shall be avoided.
We have shown that this does not work.}
...
We have already given an abstract example for that problem in \autoref{fig:orchestration:no_upper_bound}. \autoref{fig:orchestration:no_orchestration}, in addition, demonstrates this situation at a derivation of the running example.
