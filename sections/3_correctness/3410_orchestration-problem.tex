...
In the following, we show the necessity of having this unrestricted notion rather than allowing each transformation to be executed only once, as proposed by existing work~\cite{stevens2020BidirectionalTransformationLarge-SoSym}.
...
Consider the following consistency relations. For simplicity reasons, we use \modellevelconsistencyrelations (\autoref{def:modellevelconsistencyrelation}) instead of fine-grained ones:
\begin{align*}
    & 
...
\begin{proof}
    ...
    \begin{properdescription}
        ...
        \item[$max_n(\model{m}{1},\model{m}{2},\model{m}{3}) < x-1 \Rightarrow \model{m}{1},\model{m}{2},\model{m}{3}$ are not consistent to $\consistencyrelationset{CR}$:]
...
Of course, for a concrete set of transformations it may be possible that a there is an orchestration for all possible models and changes to them leading to a consistent state and only requiring each transformation to be executed once.
...
Consistency preservation rules that preserve consistency to these consistency relations need to choose one of the given options for the names of corresponding employees, residents and persons.
