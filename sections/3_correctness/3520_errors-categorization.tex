...
It can, however, be an indicator whether the algorithm fails because and artificial execution bound was reached or because a transformation cannot be applied anymore, because it is not able to process the given changes.
...
\mnote{Mistakes and faults derived from the specification levels}
...
\mnote{Technical mistakes excluded, transformations assumed to be correct}
...
\mnote{Correctness requires synchronizing transformations}
...
\mnote{Mistake is missing considering or knowledge about synchronization, leading to missing element matchings}
...
\mnote{Correctness of consistency relations can be in compabination or individual}
...
\mnote{Individual correctness of consistency relation irrelevant}
...
\mnote{No precise notion of correctness for consistency preservation rules}
...
\mnote{One possible mistake is selection of contradicting options}
...
\mnote{No complete overview of potential mistakes}
...
\mnote{Overview of dependencies between mistakes, faults and failures}
...
%    \includegraphics[angle=270, width=\textwidth]{figures/mistakes_examples_employee.pdf}
    \caption{Adaptation of consistency relations from the extended running example in \autoref{fig:compatibility:three_persons_example_extended}. Adapted from~\owncite{klare2019icmt}.}
    \label{fig:errors:mistake_effects_example_metamodels}
\end{figure}

\begin{figure}
    \centering
    \input{figures/correctness/errors/failure_examples_employee.tex}
%    \includegraphics[angle=270, width=\textwidth]{figures/mistakes_examples_employee.pdf}
    \caption[Examples for mistakes at each level]{Examples for transformation executions based on the consistency relations given in \autoref{fig:errors:mistake_effects_example_metamodels} with mistakes at each of the three levels. Numbers indicate transformation execution steps, +/- indicate element addition or removal. Adapted from~\owncite{klare2019icmt}.}
    \label{fig:errors:mistake_effects_example_cases}
\end{figure}

\mnote{Examples for mistakes at each level}
...
\mnote{Duplicate element creation by missing synchronization}
...
\mnote{Inconsistent termination by incompatibility}
The example for the \levelnetworkrelation level considers the incompatible consistency relations $R_{PE}$, $R_{PR}$ and $R_{ER}'$. Thus, the transformation developers made the mistake of not having a compatible knowledge about consistency constraints.
...
\mnote{Non-termination by contradicting option selection for consistency restoration}
...
\mnote{Incompatible constraints and contradiction option selection cannot always be distinguished}
