...
In the following section, we perform a case distinction for all possible combinations of changes, specifically for \gls{EMOF}-based models as the most common and general formalism to describe metamodels and models.
...
\paragraph{Irrelevant considerations on condition element level}
\begin{itemize}
...
\paragraph{Considerations on case distinction for co-occurring changes to corresponding elements}
Two problem cases: 
...
Wurde es geändert, ändere das korrespondierende Condition Element in d2(m2). Wurde es in d2(m2) entfernt, ist es an CPRl diese Löschung zurückzupropagieren, also tue nichts. Wurde das Element in d2(m2) geändert, haben wir ein Problem \todo{was ist dann?}
...
\subsection{Synchronizing Transformation cannot be Unidirectional}
