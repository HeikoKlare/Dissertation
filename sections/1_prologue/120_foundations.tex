...
In projective approaches, the underlying repository can, again, be seen as a model, such that views in projective approaches are projections of that model~\owncite[Fig.~5]{klare2020Vitruv-JSS}.
...
The \vitruv approach~\owncite{klare2020Vitruv-JSS} bases on the \gls{OSM} idea of having a \gls{SUM} from which projective views are derived though which the information in the \gls{SUM} can be modified.
...
The \gls{SUM} internally consists of models, which are kept consistent by model transformations, called \emph{consistency preservation rules}, and is denoted as a \vsum~\owncite[Def.~9]{klare2020Vitruv-JSS}.
The metamodel of a \vsum is denoted as a \vsumm~\owncite[Def.~10]{klare2020Vitruv-JSS}.
...
For \vitruv, we have provided a simple but sufficient formalism defining consistency~\owncite{klare2020Vitruv-JSS}.
...
This especially comprises the \emph{\mappings language}~\cite{werle2016a}, which is a bidirectional, declarative language comparable to \qvtr, and the \emph{\reactionslanguage}~\owncite{klare2016b} for defining imperative, unidirectional transformations.
...
float,%
language=reactions,
caption={[Reaction for creating classes for components]\reaction creating a \gls{UML} class for a \gls{PCM} component. Adapted from~\owncite[Lst.~2]{klare2020Vitruv-JSS}.},
...
For the complete explanation of that example, we refer to previous work~\owncite{klare2020Vitruv-JSS}.
...
% First, it is easier to think about binary than about multiary relations~\cite{stevens2017a}.
...
% Finally, we investigate of a subset of problems that can actually occur, as in a concrete scenario multiary relations may exist that cannot be expressed by sets of binary relations.
% Although we limit our considerations to the assumed scenarios, most of our findings could also be extended to a modularization into smaller multiary relations rather than binary relations.
